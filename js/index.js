  ...
  methods: {
    validateForm() {
      ...
      for (item in this.data) {
        this.error[item] = "";
        if (!this.data[item])
          (this.error[item] = "Field required"), (sendMail = false);
        else {
          if (item === "email" && !mailPattern.test(this.data[item]))
            (this.error[item] = "Invalid email"), (sendMail = false);
          if (item === "phone" && !telephonePattern.test(this.data[item]))
            (this.error[item] =
              "The telephone number must start with a + and have 11 characters without spaces"),
              (sendMail = false);
